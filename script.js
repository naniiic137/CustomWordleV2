document.addEventListener('DOMContentLoaded', function () {
    ...
    function pack(cfg){
        ...
        var flags4=(cfg.shuffle?1:0)|(cfg.decay?2:0)|(cfg.memory?4:0)|(cfg.reverse?8:0)|(cfg.minefield?16:0)|(cfg.anagram?32:0)|(cfg.sniper?64:0)|(cfg.spiral?128:0);
        var flags5=(cfg.chain?1:0)|(cfg.bookMode?2:0)|(cfg.voidMode?4:0)|(cfg.blitz?8:0);
        ...
        if(flags4)header.push(0x03,flags4);
        if(cfg.bookWords&&cfg.bookWords.length){
            var bwJson=JSON.stringify(cfg.bookWords);
            header.push(0x04,(bwJson.length>>8)&0xff,bwJson.length&0xff);
            for(var bwi=0;bwi<bwJson.length;bwi++)header.push(bwJson.charCodeAt(bwi));
        }
        if(flags5)header.push(0x05,flags5);
    ...
    function unpack(u){
        ...
        var f2=0,hintUnlock=0,f3=0,f4=0,f5=0,maxPlayers=0;
        ...
        if(i<u.length&&u[i]===0x01){
            ...
            while(i<u.length&&u[i]!==0){
                ...
                if(u[i]===0x03){i++;f4=u[i++]||0;continue;}
                if(u[i]===0x04){
                    i++;
                    if(i+1<u.length){
                        var bwLen=(u[i++]<<8)|u[i++];
                        var bwChars=[];
                        for(var bw=0;bw<bwLen&&i<u.length;bw++)bwChars.push(String.fromCharCode(u[i++]));
                        try{bookWords=JSON.parse(bwChars.join(''))||[];}catch(e){}
                    }
                    continue;
                }
                if(u[i]===0x05){i++;f5=u[i++]||0;continue;}
                break;
            }
        }
        if(i<u.length&&u[i]===0){
        ...
        return{word:w,word2:w2,hide:!!(f&1),nocol:!!(f&2),nobk:!!(f&4),one:!!(f&8),rf:!!(f&16),sd:!!(f&32),timed:!!(f&128),
               ...
               shuffle:!!(f4&1),decay:!!(f4&2),memory:!!(f4&4),reverse:!!(f4&8),minefield:!!(f4&16),anagram:!!(f4&32),sniper:!!(f4&64),spiral:!!(f4&128),
               chain:!!(f5&1),bookMode:!!(f5&2),voidMode:!!(f5&4),blitz:!!(f5&8),bookWords:bookWords,
    ...
    var shuffleMode=false,decayMode=false,memoryMode=false,reverseMode=false,minefieldMode=false,anagramMode=false,sniperMode=false,spiralMode=false;
    var chainMode=false,bookMode=false,voidMode=false,blitzMode=false;
    ...
    var minePositions=[];
    var mineHitsByRow={};
    var reverseTargetPattern='';
    var spiralRound=0,spiralLengths=[3,4,5,6];
    var spiralUsedWords=[];
    var bookWordsPool=[];
    var voidPosition=-1;
    ...
    function scoreKeyFromResult(result){
        return result.map(function(e){return e.s==='correct'?'C':e.s==='present'?'P':'A';}).join('');
    }
    function patternToEmoji(key){
        return key.split('').map(function(k){return k==='C'?'🟩':k==='P'?'🟨':'⬛';}).join('');
    }
    function genPseudoWord(len){
        var cons='BCDFGHJKLMNPQRSTVWXYZ';
        var vows='AEIOU';
        var out='';
        for(var i=0;i<len;i++){
            out+=((i%2===0)?cons:vows)[Math.floor(Math.random()*((i%2===0)?cons.length:vows.length))];
        }
        return out;
    }
    function pickWordForLength(len,usedSet){
        var list=getAbsurdleList(len).filter(function(w){return w.length===len&&/^[a-z]+$/.test(w);}).map(function(w){return w.toUpperCase();});
        if(usedSet&&usedSet.length)list=list.filter(function(w){return usedSet.indexOf(w)===-1;});
        if(list.length)return list[Math.floor(Math.random()*list.length)];
        var pool=(len===3)?['CAT','DOG','SUN','MAP','RUG','BEE','JET','PEN']:(len===6)?['POCKET','PLANET','STREAM','MARKET','CANDLE','BRIDGE','GARDEN','BOTTLE']:[];
        if(usedSet&&usedSet.length)pool=pool.filter(function(w){return usedSet.indexOf(w)===-1;});
        if(pool.length)return pool[Math.floor(Math.random()*pool.length)];
        var pseudo;
        do{pseudo=genPseudoWord(len);}while(usedSet&&usedSet.indexOf(pseudo)!==-1);
        return pseudo;
    }
    ...
    async function init(){
        ...
        if(d){
            ...
            if(cfg.bookMode&&cfg.bookWords&&cfg.bookWords.length>=5){
                bookWordsPool=cfg.bookWords.map(function(w){return String(w||'').toUpperCase().trim();}).filter(function(w){return /^[A-Z]+$/.test(w);});
                if(bookWordsPool.length>=5){
                    var roll=(parseInt(sessionStorage.getItem('book_roll')||'0',10)+1);
                    sessionStorage.setItem('book_roll',String(roll));
                    var sid=sessionStorage.getItem('wordle_session_id')||'sess';
                    var seed=(sid+':'+roll).split('').reduce(function(a,ch){return (a*31+ch.charCodeAt(0))>>>0;},7);
                    word=bookWordsPool[seed%bookWordsPool.length];
                }
            }
            ...
            shuffleMode=cfg.shuffle;decayMode=cfg.decay;memoryMode=cfg.memory;reverseMode=cfg.reverse;minefieldMode=cfg.minefield;anagramMode=cfg.anagram;sniperMode=cfg.sniper;spiralMode=cfg.spiral;
            ...
            if(minefieldMode){
                minePositions=[];
                var mineCount=Math.min(2,wordLength);
                while(minePositions.length<mineCount){
                    var pos=Math.floor(Math.random()*wordLength);
                    if(minePositions.indexOf(pos)===-1)minePositions.push(pos);
                }
            }
            if(voidMode)voidPosition=Math.floor(Math.random()*wordLength);
            if(reverseMode){
                var sample=pickWordForLength(wordLength);
                reverseTargetPattern=scoreAgainst(sample,targetWord);
            }
            if(spiralMode){
                spiralRound=0;
                spiralUsedWords=[];
                wordLength=spiralLengths[spiralRound];
                targetWord=pickWordForLength(wordLength,spiralUsedWords);
    ...
    function showModeInfoBanner(){
        var old=document.getElementById('mode-info-banner');
        if(old)old.remove();
        var txt='';
        if(reverseMode)txt='Reverse: Secret '+targetWord+' | Target '+patternToEmoji(reverseTargetPattern);
        else if(anagramMode)txt='Anagram letters: '+shuffleWord(targetWord);
        else if(spiralMode)txt='Spiral round '+(spiralRound+1)+'/4 • '+wordLength+' letters';
        else if(chainMode){
            var req=(currentRow>0&&guessGrid[currentRow-1]&&guessGrid[currentRow-1].length)?guessGrid[currentRow-1][wordLength-1].l:'Any';
            txt='Chain start letter: '+req;
        }
        if(!txt)return;
        var el=document.createElement('div');
        el.id='mode-info-banner';
        el.className='custom-hint-text';
        el.textContent=txt;
        gameContainer.insertBefore(el,gameBoard);
    }
    ...
    function showActiveModes(){
        ...
        if(shuffleMode)modes.push('🔀 Shuffle');
        if(decayMode)modes.push('⏳ Decay');
        if(memoryMode)modes.push('🧠 Memory');
        if(reverseMode)modes.push('🔄 Reverse');
        if(minefieldMode)modes.push('💣 Minefield');
        if(anagramMode)modes.push('🔡 Anagram');
        if(sniperMode)modes.push('🎯 Sniper');
        if(spiralMode)modes.push('🌀 Spiral');
        if(chainMode)modes.push('🔗 Chain');
        if(bookMode)modes.push('📖 Book Mode');
        if(voidMode)modes.push('🕳️ Void');
        if(blitzMode)modes.push('⚡ Blitz');
...
function createKeyboard(){
    ...
    function isPrimaryWinGuess(guess){
        ...
        if(reverseMode){
            return scoreAgainst(guess,targetWord)===reverseTargetPattern;
        }
    ...
    function submitGuess(){
        ...
        if(chainMode&&currentRow>0&&guessGrid[currentRow-1]&&guess[0]!==guessGrid[currentRow-1][wordLength-1].l){
            shakeRow('Your guess must start with '+guessGrid[currentRow-1][wordLength-1].l+'!');return;
        }
        if(anagramMode){
            var s1=guess.split('').sort().join(''),s2=targetWord.split('').sort().join('');
            if(s1!==s2){shakeRow('Only use the given letters!');return;}
        }
        ...
        if(shuffleMode&&guess!==targetWord){
            targetWord=shuffleWord(targetWord);
            if(reverseMode)reverseTargetPattern=scoreAgainst(pickWordForLength(wordLength),targetWord);
        }
        ...
        if(minefieldMode&&!actualWin){
            mineHitsByRow[currentRow]=minePositions.filter(function(pos){return result[pos]&&result[pos].s==='absent';});
        }else{
            mineHitsByRow[currentRow]=[];
        }
        if(decayMode&&(currentRow+1)%2===0){
            var keys=Array.from(document.querySelectorAll('.key')).filter(function(k){
                var t=k.textContent;
                return /^[A-Z0-9]$/.test(t);
            });
            keys=keys.filter(function(k){return !decayLockedKeys[k.textContent];});
            if(keys.length){
                var lock=keys[Math.floor(Math.random()*keys.length)];
                decayLockedKeys[lock.textContent]=true;
                lock.classList.add('absent');
                lock.disabled=true;
                showToast('⏳ "'+lock.textContent+'" decayed away');
            }
        }
        saveProgress();
    ...
    function applyModeScore(result,guess,rowIdx){
        ...
        /* ── Void: one position is always marked absent ── */
        if(voidMode&&voidPosition>=0&&voidPosition<result.length){
            result[voidPosition]={l:result[voidPosition].l,s:'absent'};
        }
    ...
    function checkSingleState(isWin){
        setTimeout(function(){
            ...
            if(minefieldMode&&!isWin){
                ...
                if(hits.length){
                    minePenalty=1;
                    for(var mh=0;mh<hits.length;mh++){
                        var mt=document.getElementById('tile-'+currentRow+'-'+hits[mh]);
                        if(mt){
                            mt.classList.add('mine-hit');
                            (function(t){setTimeout(function(){t.classList.remove('mine-hit');},850);})(mt);
                        }
                    }
                    showToast('💣 Mine penalty! -1 extra guess');
                }
            }
            if(isWin){
                if(spiralMode&&spiralRound<spiralLengths.length-1){
                    spiralRound++;
                    wordLength=spiralLengths[spiralRound];
                    targetWord=pickWordForLength(wordLength,spiralUsedWords);
                    spiralUsedWords.push(targetWord);
    ...
    function setupCreator(){
        ...
        var allCards=[
            ...
            {check:'shuffle-toggle',label:'shuffle-toggle-label'},{check:'decay-toggle',label:'decay-toggle-label'},
            {check:'memory-toggle',label:'memory-toggle-label'},{check:'reverse-toggle',label:'reverse-toggle-label'},
            {check:'minefield-toggle',label:'minefield-toggle-label'},{check:'anagram-toggle',label:'anagram-toggle-label'},
            {check:'sniper-toggle',label:'sniper-toggle-label'},{check:'spiral-toggle',label:'spiral-toggle-label'},
            {check:'chain-toggle',label:'chain-toggle-label'},{check:'bookmode-toggle',label:'bookmode-toggle-label'},
            {check:'void-toggle',label:'void-toggle-label'},{check:'blitz-toggle',label:'blitz-toggle-label'},
        ...
        var bmC=document.getElementById('bookmode-toggle'),bmE=document.getElementById('bookmode-extra');
        var bmInput=document.getElementById('bookmode-words-input'),bmCount=document.getElementById('bookmode-word-count');
        ...
        if(bmC&&bmE)bmC.addEventListener('change',function(){bmE.classList.toggle('hidden',!bmC.checked);});
        if(bmInput&&bmCount){
            bmInput.addEventListener('input',function(){
                var lines=bmInput.value.split(/\r?\n/).map(function(w){return w.trim();}).filter(function(w){return !!w;});
                bmCount.textContent=lines.length+' words added';
            });
        }
        ...
        document.getElementById('generate-link-button').addEventListener('click',async function(){
            ...
            var shuffleOn=document.getElementById('shuffle-toggle')&&document.getElementById('shuffle-toggle').checked;
            var decayOn=document.getElementById('decay-toggle')&&document.getElementById('decay-toggle').checked;
            var memoryOn=document.getElementById('memory-toggle')&&document.getElementById('memory-toggle').checked;
            var reverseOn=document.getElementById('reverse-toggle')&&document.getElementById('reverse-toggle').checked;
            var minefieldOn=document.getElementById('minefield-toggle')&&document.getElementById('minefield-toggle').checked;
            var anagramOn=document.getElementById('anagram-toggle')&&document.getElementById('anagram-toggle').checked;
            var sniperOn=document.getElementById('sniper-toggle')&&document.getElementById('sniper-toggle').checked;
            var spiralOn=document.getElementById('spiral-toggle')&&document.getElementById('spiral-toggle').checked;
            var chainOn=document.getElementById('chain-toggle')&&document.getElementById('chain-toggle').checked;
            var bookModeOn=document.getElementById('bookmode-toggle')&&document.getElementById('bookmode-toggle').checked;
            var voidOn=document.getElementById('void-toggle')&&document.getElementById('void-toggle').checked;
            var blitzOn=document.getElementById('blitz-toggle')&&document.getElementById('blitz-toggle').checked;
            var bookWords=(document.getElementById('bookmode-words-input')&&document.getElementById('bookmode-words-input').value||'').split(/\r?\n/).map(function(w){return w.toUpperCase().trim();}).filter(function(w){return !!w;});
            if(numberMode&&!mixedMode){
            ...
            if(minefieldOn&&word.length<2){showToast('Minefield needs at least 2 letters.');return;}
            if(bookModeOn){
                if(bookWords.length<5){showToast('Book Mode requires at least 5 words.');return;}
                if(bookWords.length>30){showToast('Book Mode supports up to 30 words.');return;}
                for(var bi=0;bi<bookWords.length;bi++){if(!/^[A-Z]{2,15}$/.test(bookWords[bi])){showToast('Book Mode words must be 2-15 letters.');return;}}
                word=bookWords[0];
            }
            if(spiralOn&&(mw||numberMode||mixedMode)){showToast('Spiral mode is only for single-word letter games.');return;}
            if((reverseOn||anagramOn||memoryOn||decayOn||minefieldOn||shuffleOn||sniperOn)&&mw){showToast('This new hard mode is single-board only.');return;}
            if((chainOn||bookModeOn||voidOn||blitzOn)&&mw){showToast('This mode is single-board only.');return;}
...
dictRestrict:document.getElementById('dictrestrict-toggle')&&document.getElementById('dictrestrict-toggle').checked,
                          ...
                          shuffle:shuffleOn,decay:decayOn,memory:memoryOn,reverse:reverseOn,minefield:minefieldOn,anagram:anagramOn,sniper:sniperOn,spiral:spiralOn,
                          chain:chainOn,bookMode:bookModeOn,voidMode:voidOn,blitz:blitzOn,bookWords:bookWords,
